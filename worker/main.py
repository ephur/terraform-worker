...
def create_table(name, region, key_id, key_secret, read_capacity=1, write_capacity=1):
    ...
    client = boto3.client('dynamodb', region_name=region, aws_access_key_id=key_id, aws_secret_access_key=key_secret)
    tables = client.list_tables()
    table_key = 'LockID'
    if name in tables['TableNames']:
        click.secho("DynamoDB lock table found, continuing.", fg='yellow')
    else:
        click.secho("DynamoDB lock table not found, creating, please wait...", fg='yellow')
        client.create_table(
            TableName=name,
            KeySchema=[
                {
        ...
        client.get_waiter('table_exists').wait(TableName=name, WaiterConfig={'Delay': 10, 'MaxAttempts': 30})


def get_aws_id(key_id, key_secret):
    """Return the AWS account ID."""
    client = boto3.client('sts', aws_access_key_id=key_id, aws_secret_access_key=key_secret)
    return client.get_caller_identity()['Account']


def ordered_config_load(stream, args, Loader=yaml.SafeLoader, object_pairs_hook=OrderedDict):
    """
...
def pipe_exec(args, stdin=None, cwd=None, env=None):
    ...
    for i in args:
        if count == 0:
            ...
            commands.append(subprocess.Popen(shlex.split(i), stdout=subprocess.PIPE,
                                             stderr=subprocess.PIPE, stdin=commands[count - 1].stdout, cwd=cwd,
                                             env=env))
        count = count + 1
