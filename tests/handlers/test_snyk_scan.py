...
class TestSnykHandler:
    ...
    def test_init(self):
        ...
        assert handler._required is True
    ...
    def test_execute_post_plan_no_changes(self):
        ...
        result = TerraformResult(
            0, "Plan has no changes".encode("utf-8"), "".encode("utf-8")
        )
        config = SnykConfig(path=snyk_path, required=True)
        handler = SnykHandler(config)
        handler.execute(
        ...
        with pytest.raises(HandlerError):
            handler.execute(
                ...
                definition=Definition(
                    name="test_def", path="apps-dev/test-def", plan_file=plan_path
                ),
                working_dir=working_dir,
                result=result,
            )
    ...
    def test_verify_snyk_args(self):
        ...
        assert args == [snyk_path, "iac", "test", test_def_dir]
