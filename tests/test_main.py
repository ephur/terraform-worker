...
class TestMain:
    ...
    def test_config_loader(self, state):
        ...
        expected_tf_vars = {"vpc_cidr": "10.0.0.0/16", "region": "us-west-2", "domain": "test.domain.com"}
    ...
    @pytest.mark.parametrize("commands, exit_code, cwd, stdin, stdout, stderr", [
        ("/bin/true", 0, None, None, "", ""),
        ("/bin/false", 1, None, None, "", ""),
        ("/bin/echo foo", 0, None, None, "foo", ""),
        ("/bin/grep foo", 0, None, "foo", "foo", ""),
        ("/bin/pwd", 0, "/tmp", None, "/tmp", ""),
        ("/bin/cat /yisohwo0AhK8Ah ", 1, None, None, "", "/bin/cat: /yisohwo0AhK8Ah: No such file or directory"),
    ...
    def test_run_pipe_exec(self, commands, exit_code, cwd, stdin, stdout, stderr):
        (return_exit_code, return_stdout, return_stderr) = worker.main.pipe_exec(commands, cwd=cwd, stdin=stdin)
